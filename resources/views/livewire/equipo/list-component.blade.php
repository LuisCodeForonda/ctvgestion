...
<div>
    <div class="flex flex-row justify-between items-center py-2">
        ...
        <a href="{{ route('componente.pdf') }}">Generar pdf</a>
        <div class="max-w-sm">
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    ...
                    @include('includes.table-sortable', ['name' => 'modelo', 'displayName' => 'Modelo'])
                    @include('includes.table-sortable', ['name' => 'serie', 'displayName' => 'Serie'])
                ...
                @foreach ($data as $item)
                    <tr wire:key="{{ $item->id }}" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td class="px-6 py-4 flex gap-4">
                            ...
                            <a href="{{ route('componente.edit', $item) }}" wire:navigate
    ...
    @if ($show)
        <x-modal-show title="Detalle del componente">
            ...
            @if (isset($componente->observaciones))
                <strong>Observaciones</strong>
                <p>{{ $componente->observaciones }}</p>
            @endif
            @if (isset($componente->modelo))
                <strong>Modelo</strong>
                <p>{{ $componente->modelo }}</p>
            @endif
            @if (isset($componente->serie))
                <strong>Serie</strong>
