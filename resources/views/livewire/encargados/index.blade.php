...
new #[Layout('layouts.app')] class extends Component {
    ...
    {
        return [
            'data' => Encargado::where('nombre', 'LIKE', '%' . $this->search . '%')
...
<div>
    @slot('header')
        <h1 class="font-bold">Encargados</h1>
    @endslot

    @if ($data->isEmpty())
        <div class="text-center mt-8">
            <p class="mb-4 text-2xl">Aún no hay encargados</p>
            <x-primary-button href="{{ route('encargados.create') }}" wire:navigate>Nuevo</x-primary-button>
        </div>
    @else
        <x-primary-button href="{{ route('encargados.create') }}" wire:navigate>Nuevo</x-primary-button>
                    ...
                    @foreach ($data as $item)
                        <tr wire:key="{{ $item->id }}"
                            ...
                            <td class="px-6 py-4 flex gap-4">
                                ...
                                <a href="{{ route('encargados.edit', $item) }}" wire:navigate
    ...
    @if ($show)
        <x-modal-show title="Detalle del marca">
            ...
            @if ($encargado->carnet)
                ...
                <p>{{ $encargado->carnet }}</p>
            @endif

            @if ($encargado->celular)
                <strong>Celular</strong>
                <p>{{ $encargado->celular }}</p>
            @endif
    ...
    @if ($showDelete)
        <x-modal-destroy-confirm>
            <p class="mb-4">{{ $encargado->nombre }}</p>
        </x-modal-destroy-confirm>
    @endif
</div>
